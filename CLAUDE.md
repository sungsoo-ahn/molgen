...
2. **Check GPU availability before running GPU tasks** (see GPU Tasks section below)
3. Run via script in `scripts/`:
   ```bash
   bash scripts/run_experiment.sh
   ```
4. Results appear in the `output_dir` specified in config

### GPU Tasks

**Always check GPU availability before running training or generation tasks:**
...
**Quick GPU check command:**
```bash
# One-liner to verify GPU is ready
nvidia-smi --query-gpu=name,memory.total,memory.free --format=csv
```

**Before running training:**
1. Verify GPU is available and has sufficient memory
2. Check no other processes are using the GPU (or account for shared usage)
3. For large models, ensure at least 8GB free GPU memory

**If GPU is not available:**
- Training will fall back to CPU (very slow)
- Consider using a machine with GPU or cloud GPU instances
- Reduce batch size if running on limited GPU memory
...
```bash
# Check GPU availability (run before training)
nvidia-smi
...
- **CUDA out of memory**: Reduce `batch_size` in config, or use gradient checkpointing
- **CUDA not available**: Check `nvidia-smi` works; reinstall PyTorch with CUDA support if needed
- **Training very slow**: Verify GPU is being used (check `nvidia-smi` during training for GPU utilization)
